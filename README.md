...
오버레이는 `Show overlay`로 켜고 끌 수 있으며, 전체 번역 히스토리를 실시간 채팅창처럼 보여줍니다. 새 번역이 들어오면 최신 항목으로 자동 이동하지만, 오버레이 안에서 스크롤해 이전 내용을 확인할 수 있습니다. 화면 위에서 직접 드래그해 옮길 수 있고, 놓은 위치는 `manual` 위치로 저장됩니다. 오른쪽 아래 모서리를 드래그하면 오버레이 크기를 바로 조절할 수 있으며, `Overlay width`와 `Overlay height` 값에도 반영됩니다.
번역들은 `History` 탭에도 원문/번역 쌍으로 계속 쌓이며, `Max`로 전체 보관 개수를 조절하고 `Clear history`로 비울 수 있습니다. 오버레이도 이 전체 보관 히스토리를 그대로 보여줍니다.
